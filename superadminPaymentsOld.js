...
async function upsertWallet(db, client_id, amount) {
  const selectWalltet = `select balance form client_wallets where client_id=$1`;
  const result = await db.query(selectWalltet, [client_id]);
  if (result.rowCount == 0) {
    const q = `
    INSERT INTO client_wallets (client_id, balance, updated_at)
    VALUES ($1, $2, NOW()) RETURNING balance `;
    await db.query(q, [amount, client_id]);
    //insert
  } else {
    let balance = result.rows[0].balance;
    let updatedBalance = balance + amount;
    await db.query(
      `UPDATE client_wallets SET balance = balance + $1, updated_at = NOW() WHERE client_id = $2 RETURNING balance`,
      [updatedBalance, client_id]
    );
  }
}
router.post(
  ...
  async (req, res) => {
    try {
      ...
      return res.json({
        success: true,
        message:
          "Plan switched successfully (zero payable). Any leftover credit added to wallet.",
        ...
        subscription: {
          ...
          current_period_start: subscriptionResult.current_period_start
            ? new Date(subscriptionResult.current_period_start).toISOString()
            : null,
          current_period_end: subscriptionResult.current_period_end
            ? new Date(subscriptionResult.current_period_end).toISOString()
            : null,
        },
      });
    } catch (err) {
      ...
      res
        ...
        .json({
          success: false,
          message: "Failed to confirm switch",
          detail: err.message,
        });
    }
  }
